...
// <!----------------Sorry but due to laziness creating two classes here---------------------------------!>
...
class AddTagButton extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps != this.props) {
      if ('tagId' in this.props) {
        if (this.props.tagId) {
          //if tags available in this.props use it
          let props = null
          if (this.props.tags && this.props.tags.length && this.props.tags.length > 0) {
            props = this.props
          ...
          const tagValue = this.getTagButtonValue(this.props.tagId, props)
          if (tagValue) {
            this.setState({
    ...
    return (
      <div className="container-add-tag">
        ...
        <Dialog
          ...
          contentStyle={{ maxWidth: 400, textAlign: "center" }}
        >
